...
module toec_dual_stabilizer (
    input wire sys_clk, clk_glitch_line, ext_rst_n,
    input wire [7:0] vdd_core_voltage,
    input wire [31:0] poly_space_coord, matrix_energy_sig,
    input wire split_brain_flag,
    input wire [31:0] vasp_theta_param, noisy_distance_q8,
    ...
    always @(posedge sys_clk or negedge ext_rst_n) begin
        if (!ext_rst_n) begin
            ...
            sync_ff1 <= clk_glitch_line; sync_ff2 <= sync_ff1;
            sync_ff3 <= sync_ff2; glitch_edge_reg <= sync_ff3;
        end
    end
